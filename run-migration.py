...
def extract_backup(args):
    ...
    # for dumps not containing a filestore
    os.makedirs("source/filestore", exist_ok=True)
...
RUN cd /odoo && for yml_file in *.yml; do \
    ...
    pip install -U $(for requirement in */requirements.txt *.txt; do \
        if [ -f $requirement ]; then echo -n " -r $requirement"; fi; \
    done) ./odoo
USER openupgrade
CMD sleep infinity
"""
...
supported_versions = ["11.0"] + sorted(
    version
